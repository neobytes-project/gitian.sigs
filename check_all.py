...
def verify(version):
    ...
    # Checkout proper neobytes tag
    os.chdir('neobytes')

    ignore_dir = ['.git', '.github', 'neobytes', 'gitian-builder', 'archive', 'gitian-keys']
    ...
    result = subprocess.call(
        ['bin/gverify', '-v', '-d', '../', '-r', version, '../neobytes/contrib/gitian-descriptors/' + gitian_yml])
    assert result == 0
